...
### 2026-09-06 — 项目 rail 语义收口

- 将首屏右侧的重点项目 rail 从带 `aria-label` 的通用容器改为原生 `nav`，保留既有 hover、focus、active signal 和案例锚点行为。
- 这样首屏同时拥有“求职方向入口”和“重点项目入口”两层可识别导航，读屏和键盘用户不需要猜测视觉面板的用途。
- 新增渲染断言锁定 `重点项目` 导航和两个案例入口，没有引入新依赖或改变内容事实。

### 2026-09-06 — 外部方向审查后的招聘阅读收口

- 采纳独立审查中可由现有证据支持的四项小改动：项目卡前置 `MY CONTRIBUTION`、统一为“两个业务主案例，一个 AI 工作流案例”、首页每个项目只突出一条主证据并将其余证据收进原生 `details`、封面指标补充 `LOCAL REPLAY` / `SYNTHETIC EVAL` 等范围词。
